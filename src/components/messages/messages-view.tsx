    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div className="flex gap-2">
          <button onClick={() => setTab('recus')} className={`px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${tab === 'recus' ? 'bg-orange-500/20 text-orange-400 border-orange-500/30' : 'bg-black/40 text-orange-200/60 border-orange-500/10 hover:text-white'}`}>
            Reçus {nonLus > 0 && <span className="ml-1 bg-orange-500 text-black rounded-full px-1.5 text-[11px]">{nonLus}</span>}
          </button>
          <button onClick={() => setTab('envoyes')} className={`px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${tab === 'envoyes' ? 'bg-orange-500/20 text-orange-400 border-orange-500/30' : 'bg-black/40 text-orange-200/60 border-orange-500/10 hover:text-white'}`}>
            Envoyés
          </button>
        </div>
        <button onClick={() => setComposeOpen(true)} className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-black rounded-xl px-4 py-2 font-semibold text-sm transition-colors">
          ...
          {displayed.map((m) => (
            <button key={m.id} onClick={() => openMessage(m)} className="w-full flex items-start gap-3 px-5 py-3.5 text-left hover:bg-orange-500/5 transition-colors">
              <div className="mt-0.5 shrink-0">
                {tab === 'recus' && !m.lu ? <Mail size={16} className="text-orange-400" /> : <MailOpen size={16} className="text-zinc-500" />}
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <p className={`text-sm truncate ${tab === 'recus' && !m.lu ? 'text-white font-semibold' : 'text-orange-100/70'}`}>{m.sujet}</p>
                </div>
                <p className="text-xs text-orange-200/40 mt-0.5 truncate">
          ...
          <div className="bg-zinc-950 border border-orange-500/20 rounded-2xl p-7 w-full max-w-lg">
            <div className="flex items-start justify-between gap-3 mb-4">
              <div className="min-w-0">
                <h2 className="text-lg font-bold text-white">{open.sujet}</h2>
                <p className="text-xs text-orange-200/40 mt-1">De {open.fromNom} · À {open.toNom} · {new Date(open.createdAt).toLocaleString('fr-FR', { dateStyle: 'medium', timeStyle: 'short' })}</p>
              </div>
              <button onClick={() => setOpen(null)} className="text-zinc-500 hover:text-white shrink-0"><X size={20} /></button>
            </div>
            <p className="text-orange-100/80 text-sm leading-relaxed whitespace-pre-wrap">{open.corps}</p>
            <div className="flex justify-end mt-6">
              <button onClick={() => handleDelete(open)} className="flex items-center gap-2 text-xs text-zinc-400 hover:text-red-400 transition-colors"><Trash2 size={13} /> Supprimer</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-orange-500/20 rounded-2xl p-7 w-full max-w-lg">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setComposeOpen(false)} className="text-zinc-500 hover:text-white"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-orange-200/60 text-xs font-medium block mb-1.5">Destinataire</label>
              ...
              <div>
                <label className="text-orange-200/60 text-xs font-medium block mb-1.5">Objet</label>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setComposeOpen(false)} disabled={sending} className="flex-1 border border-orange-500/20 text-orange-200/60 rounded-xl py-2.5 text-sm hover:border-orange-500/40 hover:text-white transition-colors disabled:opacity-50">Annuler</button>
                <button onClick={handleSend} disabled={sending || !form.toUid || !form.sujet.trim() || !form.corps.trim()} className="flex-1 flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-40 text-black font-semibold rounded-xl py-2.5 text-sm transition-colors">
