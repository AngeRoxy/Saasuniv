...
export default function RevenuePage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-white">Revenus de la plateforme</h1>
        <p className="text-orange-200/40 text-sm mt-1">MRR / ARR estimés à partir des abonnements actifs réels.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-zinc-950 border border-orange-500/10 rounded-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-orange-500/10"><h2 className="text-sm font-semibold text-white">Revenus par université</h2></div>
          <table className="w-full text-sm">
            ...
            <tbody>
              {universities.length === 0 ? (
                <tr><td colSpan={4} className="px-4 py-12 text-center text-orange-200/30 text-sm">Aucune université inscrite.</td></tr>
              ) : universities.map((u) => (
                <tr key={u.id} className="border-t border-orange-500/5 hover:bg-orange-500/5 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center text-orange-100/60">{planLabel(u.plan ?? '')}</td>
                  <td className="px-4 py-3 text-right text-orange-100/70 whitespace-nowrap">{monthlyRevenue(u.plan ?? '') > 0 ? fmtFCFA(monthlyRevenue(u.plan ?? '')) : '—'}</td>
        ...
        <div className="bg-zinc-950 border border-orange-500/10 rounded-xl p-6 h-fit">
          ...
          <div className="space-y-4">
            {repartition.map((r) => (
              <div key={r.plan} className="p-4 rounded-xl border border-orange-500/10 bg-black/20">
                <div className="flex items-center justify-between mb-1">
                  <span className="font-semibold text-orange-300">{r.plan}</span>
                  <span className="text-orange-100/60 text-xs">{r.clients} client{r.clients !== 1 ? 's' : ''}</span>
                </div>
                <p className="text-white font-bold">{fmtFCFA(r.mrr)}</p>
                <p className="text-orange-200/40 text-xs mt-0.5">/ mois</p>
              </div>
            ))}
          </div>
        </div>
