...
export default function ParentProfilePage() {
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <Baby className="h-4 w-4 text-blue-600 dark:text-orange-400" />
          <h2 className="text-sm font-semibold text-zinc-800 dark:text-orange-100/80">Mes enfants</h2>
        </div>
        <div className="rounded-xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-orange-500/10 px-6 py-2">
          {enfants.length === 0 ? (
            <p className="py-4 text-sm text-zinc-500 dark:text-orange-300/30">Aucun enfant lié à votre compte.</p>
          ) : (
            enfants.map((enfant) => (
              <div key={enfant.uid} className="flex items-center gap-4 py-3 border-b border-orange-500/5 last:border-0">
                <div className="rounded-lg bg-orange-500/10 p-2 shrink-0">
                  <GraduationCap className="h-4 w-4 text-blue-600 dark:text-orange-400" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-zinc-900 dark:text-white font-medium truncate">{enfant.displayName}</p>
                  <p className="text-xs text-zinc-500 dark:text-orange-300/40 truncate">
