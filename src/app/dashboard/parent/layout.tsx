...
const navLinks = [
  { href: '/dashboard/parent', label: 'Accueil', icon: Home },
  { href: '/dashboard/parent/schedule', label: 'Emploi du temps', icon: CalendarClock },
  { href: '/dashboard/parent/examens', label: 'Examens', icon: ClipboardList },
  { href: '/dashboard/parent/grades', label: 'Notes', icon: FileText },
  { href: '/dashboard/parent/absences', label: 'Absences', icon: AlertCircle },
  { href: '/dashboard/parent/payments', label: 'Paiements', icon: CreditCard },
  { href: '/dashboard/parent/messages', label: 'Messages', icon: MessageSquare },
  { href: '/dashboard/parent/annonces', label: 'Annonces', icon: Bell },
  { href: '/dashboard/parent/profile', label: 'Mon profil', icon: User },
]
...
export default function ParentLayout({ children }: { children: React.ReactNode }) {
  ...
  const displayName = profile?.displayName ?? user?.displayName ?? user?.email ?? 'Parent'
  const universityId = profile?.universityId ?? '—'

  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-black">
      <aside className="w-64 bg-white border-r border-zinc-200 dark:bg-zinc-950 dark:border-orange-500/10 fixed h-full flex flex-col z-10">
        <div className="flex items-center gap-2 px-6 py-5 border-b border-zinc-200 dark:border-orange-500/10">
          <GraduationCap className="h-6 w-6 text-orange-500" />
          <span className="text-lg font-bold text-zinc-900 dark:text-white tracking-tight">GestUniv</span>
        </div>

        <nav className="flex-1 py-4 overflow-y-auto">
          <p className="px-6 py-2 text-[10px] uppercase tracking-widest text-zinc-500 dark:text-orange-300/30 mb-1">Parent</p>
      ...
      <main className="ml-64 p-8 bg-[#fafafa] dark:bg-black min-h-screen">
