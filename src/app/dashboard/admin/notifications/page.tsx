  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-3">
        <h2 className="text-sm font-semibold text-zinc-900 dark:text-white flex items-center gap-2"><Bell size={15} className="text-blue-600 dark:text-orange-400" /> Annonces publiées</h2>
        {loading ? (
          ...
          <div className="text-center py-16 text-zinc-500 dark:text-orange-200/30 text-sm">Aucune annonce publiée pour l’instant.</div>
        ) : (
          annonces.map((a) => (
            <div key={a.id} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-orange-500/10 rounded-xl p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-zinc-900 dark:text-white font-semibold text-sm">{a.titre}</p>
                    <span className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full bg-orange-500/10 border border-orange-500/20 text-blue-700 dark:text-orange-300"><Users size={9} /> {CIBLE_LABEL[a.destinataire]}</span>
                  </div>
                  <p className="text-zinc-800 dark:text-orange-100/60 text-xs leading-relaxed mt-1.5">{a.message}</p>
                  <p className="text-zinc-500 dark:text-orange-200/30 text-[11px] mt-2">{a.auteur} · {new Date(a.createdAt).toLocaleString('fr-FR', { dateStyle: 'medium', timeStyle: 'short' })}</p>
                </div>
      ...
      <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-orange-500/10 rounded-xl p-6 h-fit">
        <h2 className="text-sm font-semibold text-zinc-900 dark:text-white mb-4 flex items-center gap-2"><Send size={15} className="text-blue-600 dark:text-orange-400" /> Nouvelle annonce</h2>
        <div className="space-y-4">
          <div>
            ...
            <input value={titre} onChange={(e) => setTitre(e.target.value)} placeholder="Objet de l'annonce"
              className="w-full bg-zinc-50 dark:bg-black/40 border border-orange-500/20 rounded-xl px-4 py-3 text-zinc-900 dark:text-white text-sm focus:outline-none focus:border-orange-400/60 placeholder:text-zinc-500 dark:placeholder:text-orange-200/25" />
          </div>
          <div>
            ...
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={4} placeholder="Contenu…"
              className="w-full bg-zinc-50 dark:bg-black/40 border border-orange-500/20 rounded-xl px-4 py-3 text-zinc-900 dark:text-white text-sm focus:outline-none focus:border-orange-400/60 resize-none placeholder:text-zinc-500 dark:placeholder:text-orange-200/25" />
          </div>
          <div>
            <label className="text-zinc-600 dark:text-orange-200/60 text-xs font-medium block mb-1.5">Destinataires</label>
            <select value={cible} onChange={(e) => setCible(e.target.value as AnnonceCible)}
              className="w-full bg-white dark:bg-zinc-900 border border-orange-500/20 rounded-xl px-4 py-3 text-zinc-900 dark:text-white text-sm focus:outline-none focus:border-orange-400/60">
              {CIBLES.map((c) => <option key={c} value={c}>{CIBLE_LABEL[c]}</option>)}
            </select>
          </div>
          <button onClick={handleSend} disabled={sending || !titre.trim() || !message.trim()}
            className="w-full flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-40 text-white font-semibold rounded-xl py-3 text-sm transition-colors">
