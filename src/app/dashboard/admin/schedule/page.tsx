...
export default function SchedulePage() {
  ...
  // Maintenance ponctuelle (dev) : nettoyage des créneaux déjà orphelins.
  ...
  // Maintenance ponctuelle : supprime créneaux ET examens dont la filière / le
  // semestre / la matière ont disparu, et vide l'enseignant/surveillant introuvable
  // (créneau/examen conservé).
  async function handleCleanupOrphans() {
    ...
    try {
      const r = await cleanupOrphanData(universityId)
      await refreshCreneaux()
      const s = (n: number) => (n !== 1 ? 's' : '')
      const cDel = r.creneaux.deletedFiliere + r.creneaux.deletedSemestre + r.creneaux.deletedMatiere
      const eDel = r.examens.deletedFiliere + r.examens.deletedSemestre + r.examens.deletedMatiere
      setCleanupMsg(
        `Créneaux : ${cDel} supprimé${s(cDel)} ` +
        `(${r.creneaux.deletedFiliere} filière, ${r.creneaux.deletedMatiere} matière, ${r.creneaux.deletedSemestre} semestre), ` +
        `${r.creneaux.personneCleared} enseignant${s(r.creneaux.personneCleared)} retiré${s(r.creneaux.personneCleared)}. ` +
        `Examens : ${eDel} supprimé${s(eDel)} ` +
        `(${r.examens.deletedFiliere} filière, ${r.examens.deletedMatiere} matière, ${r.examens.deletedSemestre} semestre), ` +
        `${r.examens.personneCleared} enseignant/surveillant retiré${s(r.examens.personneCleared)}.`
      )
    } catch (err) {
      setCleanupError(`Échec du nettoyage : ${err instanceof Error ? err.message : 'erreur inconnue'}.`)
    } finally {
      setCleaning(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Maintenance ponctuelle (DEV uniquement) : nettoyage des créneaux déjà
          orphelins laissés par d'anciennes suppressions sans cascade. À retirer
          une fois les données assainies — la cascade automatique empêche désormais
          l'apparition de nouveaux orphelins. */}
      {process.env.NODE_ENV === 'development' && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 space-y-2">
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-[11px] uppercase tracking-wider text-amber-300/70 font-semibold">
              Maintenance (dev)
            </span>
            <button
              onClick={handleCleanupOrphans}
              disabled={cleaning}
              className="flex items-center gap-2 bg-zinc-900 hover:bg-zinc-800 border border-white/10 text-zinc-200 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {cleaning && <span className="w-3.5 h-3.5 border-2 border-zinc-500 border-t-transparent rounded-full animate-spin" />}
              {cleaning ? 'Nettoyage…' : 'Nettoyer les données orphelines (créneaux + examens)'}
            </button>
          </div>
          {cleanupMsg && <p className="text-xs text-emerald-300 leading-relaxed">{cleanupMsg}</p>}
          {cleanupError && <p className="text-xs text-red-300 leading-relaxed">{cleanupError}</p>}
        </div>
      )}
      ...
      {filieres.length === 0 || semestres.length === 0 ? (
        ...
        <>
          {/* Sélecteurs */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-zinc-950 border border-orange-500/10 rounded-xl p-4">
            <div>
                          ...
                          <div key={c.id} className="group rounded-lg bg-orange-500/5 border border-orange-500/15 p-2.5">
                            <div className="flex items-start justify-between gap-1">
                              ...
                              <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => openEdit(c)} className="p-1 rounded text-zinc-400 hover:text-orange-400" title="Modifier"><Pencil size={11} /></button>
                                <button onClick={() => setDeleteTarget(c)} className="p-1 rounded text-zinc-400 hover:text-red-400" title="Supprimer"><Trash2 size={11} /></button>
                              </div>
                            </div>
                            ...
                            {c.enseignant ? (
                              <p className="text-[11px] text-zinc-400 flex items-center gap-1"><User size={9} /> {c.enseignant}</p>
                            ) : (
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-orange-500/20 rounded-2xl p-7 w-full max-w-lg">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-white">{editId ? 'Modifier le créneau' : 'Ajouter un créneau'}</h2>
              ...
              <div>
                ...
                {matieres.length === 0 && (
                  <p className="text-[11px] text-orange-200/40 mt-1.5">
                    ...
                    <Link href={`/dashboard/admin/filieres/${filiereId}`} className="text-orange-400 underline hover:text-orange-300">cette filière</Link>.
                  </p>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-red-500/20 rounded-2xl p-7 w-full max-w-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-base font-bold text-white">Supprimer ce créneau ?</h2>
            </div>
