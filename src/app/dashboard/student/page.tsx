  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl bg-orange-950/30 border border-orange-500/10 p-6">
        <p className="text-xs uppercase tracking-widest text-orange-300/40 mb-1">Bienvenue</p>
        <h1 className="text-2xl font-bold text-white mb-1">{displayName}</h1>
        <div className="flex items-center gap-4 text-sm text-orange-200/50">
          <span>Université : <span className="text-orange-400 font-mono">{universityId ?? '—'}</span></span>
          <span>·</span>
          <span>Email : <span className="text-orange-300/70">{user?.email}</span></span>
        </div>
      </div>
