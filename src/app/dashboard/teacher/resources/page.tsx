  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Ressources pédagogiques</h1>
          <p className="text-orange-200/40 text-sm mt-1">Partagez des documents et liens avec vos étudiants.</p>
        </div>
        <button onClick={openAdd} className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-black rounded-xl px-4 py-2 font-semibold text-sm transition-colors">
          <Plus size={16} /> Ajouter
        </button>
      </div>

      {ressources.length === 0 ? (
        <div className="text-center py-20 text-orange-200/30 text-sm flex flex-col items-center gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {ressources.map((r) => (
            <div key={r.id} className="bg-zinc-950 border border-orange-500/10 rounded-xl p-5 flex flex-col gap-2">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <Link2 size={14} className="text-orange-400 shrink-0" />
                  <h3 className="text-white font-semibold text-sm truncate">{r.titre}</h3>
                </div>
                <button onClick={() => setDeleteTarget(r)} className="shrink-0 text-zinc-500 hover:text-red-400"><Trash2 size={13} /></button>
              </div>
              {r.description && <p className="text-orange-100/50 text-xs line-clamp-2">{r.description}</p>}
              <div className="flex flex-wrap gap-1.5">
                {r.filiereId && <span className="text-[11px] bg-orange-500/10 border border-orange-500/20 text-orange-300 rounded-full px-2 py-0.5">{filiereNom(r.filiereId) ?? 'Filière'}{r.niveau ? ` · ${r.niveau}` : ''}</span>}
                {!r.filiereId && <span className="text-[11px] bg-white/5 border border-white/10 text-zinc-400 rounded-full px-2 py-0.5">Toutes filières</span>}
                {r.matiere && <span className="text-[11px] bg-white/5 border border-white/10 text-zinc-400 rounded-full px-2 py-0.5">{r.matiere}</span>}
              </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-orange-500/20 rounded-2xl p-7 w-full max-w-lg">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-white">Nouvelle ressource</h2>
              <button onClick={() => setModalOpen(false)} className="text-zinc-500 hover:text-white"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              ...
              <div>
                <label className="text-orange-200/60 text-xs font-medium block mb-1.5">Lien (URL)</label>
                <input value={form.url} onChange={(e) => setForm((f) => ({ ...f, url: e.target.value }))} placeholder="https://…" className={inputCls} />
              </div>
              <div>
                <label className="text-orange-200/60 text-xs font-medium block mb-1.5">Description (option.)</label>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-orange-200/60 text-xs font-medium block mb-1.5">Filière</label>
                ...
                <div>
                  <label className="text-orange-200/60 text-xs font-medium block mb-1.5">Niveau</label>
                ...
                <div>
                  <label className="text-orange-200/60 text-xs font-medium block mb-1.5">Matière</label>
                  <input value={form.matiere} onChange={(e) => setForm((f) => ({ ...f, matiere: e.target.value }))} placeholder="Option." className={inputCls} />
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setModalOpen(false)} disabled={saving} className="flex-1 border border-orange-500/20 text-orange-200/60 rounded-xl py-2.5 text-sm hover:border-orange-500/40 hover:text-white transition-colors disabled:opacity-50">Annuler</button>
                <button onClick={handleSave} disabled={saving || !form.titre.trim() || !form.url.trim()} className="flex-1 flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-40 text-black font-semibold rounded-xl py-2.5 text-sm transition-colors">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-red-500/20 rounded-2xl p-7 w-full max-w-sm">
            <h2 className="text-base font-bold text-white mb-2">Supprimer cette ressource ?</h2>
