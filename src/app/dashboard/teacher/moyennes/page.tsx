...
const selectCls = 'w-full bg-zinc-900 border border-orange-500/20 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-orange-400/60 disabled:opacity-50'
...
export default function TeacherMoyennesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Calculator size={22} className="text-orange-400" /> Moyennes générales</h1>
        <p className="text-orange-200/40 text-sm mt-1">
          La moyenne est calculée automatiquement à partir des notes. Vous pouvez la <strong className="text-orange-300">forcer manuellement</strong> en cas de besoin (la valeur saisie prime).
        </p>
      </div>

      {filieres.length === 0 || semestres.length === 0 ? (
        <div className="text-center py-16 text-orange-200/30 text-sm">Créez d’abord une filière (avec ses niveaux) et un semestre.</div>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-zinc-950 border border-orange-500/10 rounded-xl p-4">
            ...
            <div>
              <label className="text-orange-200/60 text-xs font-medium block mb-1.5">Semestre</label>
          ...
          {!ready ? (
            <div className="text-center py-16 text-orange-200/30 text-sm">Sélectionnez filière, niveau et semestre.</div>
          ) : rowsLoading ? (
            ...
            <div className="text-center py-16 text-orange-200/30 text-sm">Aucun étudiant dans cette filière / niveau.</div>
          ) : (
            <>
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-black rounded-xl px-4 py-2 font-semibold text-sm transition-colors">
                    ...
                    {rows.map((r) => {
                      ...
                      return (
                        <tr key={r.studentUid} className="border-t border-orange-500/5 hover:bg-orange-500/5 transition-colors">
                          <td className="px-4 py-2.5 text-orange-100/80 font-medium">{r.nom}</td>
                          <td className="px-4 py-2.5 text-center text-zinc-400">{r.auto === null ? '—' : `${r.auto.toFixed(2)}/20`}</td>
                          <td className="px-4 py-2.5 text-center">
                            <input type="number" min={0} max={20} step={0.01} value={r.manuelle}
                              ...
                              className="w-24 bg-black/40 border border-orange-500/20 rounded-lg px-2 py-1 text-center text-sm text-white focus:outline-none focus:border-orange-400/60 placeholder:text-orange-200/20" />
                          </td>
                          <td className="px-4 py-2.5 text-center">
                            {effective === null ? <span className="text-orange-200/20">—</span> : (
                              <span className="inline-flex items-center gap-2">
                                <span className={`font-bold ${effective < 10 ? 'text-red-400' : 'text-white'}`}>{effective.toFixed(2)}/20</span>
                                {mention && <span className={`text-[11px] font-semibold border rounded-full px-2 py-0.5 ${mention.cls}`}>{mention.abbr}</span>}
                                {manuelle !== null && <span className="text-[10px] text-amber-400 border border-amber-500/30 rounded px-1">forcée</span>}
                              </span>
                            )}
                          </td>
                        </tr>
