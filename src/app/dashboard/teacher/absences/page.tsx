      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <CalendarX size={22} className="text-orange-400" />
            Absences
          </h1>
          <p className="text-orange-200/40 text-sm mt-1">Les absences de vos cours. La justification est gérée par l’administration.</p>
        </div>
        {mesCreneaux.length > 0 && (
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-orange-500 hover:bg-orange-600 text-black text-sm font-semibold transition-colors shrink-0">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-orange-500/20 rounded-2xl p-7 w-full max-w-md">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-white">Marquer une absence</h2>
              <button onClick={closeModal} className="text-zinc-500 hover:text-white"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-orange-200/60 text-xs font-medium block mb-1.5">Créneau</label>
              ...
              <p className="text-[11px] text-orange-200/40">L’absence est enregistrée comme <span className="text-red-400/80">non justifiée</span>. Seule l’administration peut la justifier.</p>
